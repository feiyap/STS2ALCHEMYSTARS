...
internal static class ValencinaWarAmbushStartBannerPatch
{
	private static void Postfix(NCombatStartBanner __instance)
	{
		...
		{
			((Node)__instance).GetNode<MegaLabel>(NodePath.op_Implicit("Label")).SetTextAutoSize(((EncounterModel)ModelDb.Encounter<WarAmbushEncounter>()).Title.GetFormattedText());
		}
	}
}
