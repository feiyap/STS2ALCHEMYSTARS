...
public sealed class TremorPower : ValencinaPower
{
	...
	public override async Task AfterSideTurnEnd(PlayerChoiceContext choiceContext, CombatSide side, IEnumerable<Creature> participants)
	{
		...
		if (((PowerModel)this).Owner != null && side == ((PowerModel)this).Owner.Side && ((PowerModel)this).Amount > 0)
		{
			...
			if (num <= 0)
			{
				...
				MainFile.Logger.Info("[TremorPower] removed from " + ((PowerModel)this).Owner.Name + " at end of turn.", 1);
				return;
			}
			SetStacks(num);
			MainFile.Logger.Info($"[TremorPower] {((PowerModel)this).Owner.Name} -> {num}.", 1);
		}
	}
}
